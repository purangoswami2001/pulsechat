...
// WSHandler upgrades the connection to WebSocket and registers the client to the Hub.
func WSHandler(hub *chat.Hub, database *db.DB, jwtSecret string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !notifyOnly {
			canAccess, accessErr := database.CanAccessRoom(r.Context(), roomID, claims.UserID)
		...
		avatarURL, err := database.GetUserAvatarURL(r.Context(), claims.UserID)
		if err != nil {
			avatarURL = ""
		}
		...
		if notifyOnly {
			client = chat.NewNotifyClient(hub, conn, claims.UserID, claims.Username, avatarURL)
		} else {
			client = chat.NewClient(hub, conn, roomID, claims.UserID, claims.Username, avatarURL)
		}
